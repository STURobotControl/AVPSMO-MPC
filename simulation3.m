% S-shaped trajectory, compared to the other four observers
...
%f=10*[sign(w(1));sign(w(2));sign(w(3));sign(w(4))]; 
...
K212 = 1;
K213 = 1;
% K21 = diag([k211;k212;k213]);
K221 = 0.1;
K222 = 0.1;
K223 = 0.1;
% K22 = diag([k221;k222;k223]);
K231 = 3;
K232 = 3;
K233 = 3;
% K23 = diag([k231;k232;k233]);
K241 = 0.05;
K242 = 0.05;
K243 = 0.05;
% K24 = diag([k241;k242;k243]);
rol = zeros(3,N);
...
f_obs = zeros(3, N);  
s = zeros(3, N);  
p_obs = zeros(3, N);  % Position observations
    % Comparison Observer 1：
    f_obs1 = zeros(3, N);
    s1 = zeros(3, N);
    p_obs1 = zeros(3, N);
    e1 = zeros(3,N);
    ...
    % Comparison Observer 3：
    p_obs3 = zeros(3, N);
    f_obs3 = zeros(3, N);
    s3 = zeros(3,N);
    
    % Comparison Observer 4：
    p_obs4 = zeros(3, N);
    f_obs4 = zeros(3, N);
    s4 = zeros(3,N);
...
hold on;box on;
plot(REF(1,1:N),REF(2,1:N),'-','LineWidth',2);
plot(X_K(1,1:N),X_K(2,1:N),'--','LineWidth',2);
plot(X_K1(1,1:N),X_K1(2,1:N),':','LineWidth',2);
plot(X_K2(1,1:N),X_K2(2,1:N),':','LineWidth',2);
plot(X_K3(1,1:N),X_K3(2,1:N),':','LineWidth',2);
plot(X_K4(1,1:N),X_K4(2,1:N),':','LineWidth',2);
plot(X_K(1,1),X_K(2,1),'rd','LineWidth',2);
legend("Reference","AVPSMO-MPC","ATSMDO-MPC","NESO-MPC","DPSMO-MPC","SMDO-MPC",'FontSize',14,'location','northwest','Interpreter', 'latex');
     xlim([3.5,15]);
     ylim([1.3,7.5]);
xlabel('X (m)','FontSize',16,'Fontname','Times');
ylabel('Y (m)','FontSize',16,'Fontname','Times');
%创建一个图窗内的图窗（左间距比例，下间距比例，宽，高）
subcirc = axes('Position',[0.48 0.54 0.08 0.08]); 
...
annotation('arrow',[0.57 0.66],[0.565 0.5]);  % 画一个箭头（箭头样式、x轴指向、y轴指向）
%创建一个图窗内的图窗（左间距比例，下间距比例，宽，高）
subfig = axes('Position',[0.58 0.2 0.28 0.28]);
axes(subfig);  % 不知道有啥用
set(gca,'FontSize',12, 'FontName','Times New Roman');  % 不知道有啥用
hold on; box on;
plot(REF(1,1:N),REF(2,1:N),'-','LineWidth',2);  % 在新建的小图窗内画图
plot(X_K(1,1:N),X_K(2,1:N),'--','LineWidth',2);
plot(X_K1(1,1:N),X_K1(2,1:N),':','LineWidth',2);
plot(X_K2(1,1:N),X_K2(2,1:N),':','LineWidth',2);
plot(X_K3(1,1:N),X_K3(2,1:N),':','LineWidth',2);
plot(X_K4(1,1:N),X_K4(2,1:N),':','LineWidth',2);
set(subfig,'xlim',[9.390,9.393],'ylim',[4.940,4.970],'color','none');  % 定义小图窗内的坐标范围
hold off;

figure("name","position erorr")
for i = 1:3
    subplot (3,1,i);
    hold on;

    plot(REF(i,1:N)-X_K(i,1:N),'LineWidth',1);
    
    plot(REF(i,1:N)-X_K1(i,1:N),'--','LineWidth',1);
    
    plot(REF(i,1:N)-X_K2(i,1:N),'-.','LineWidth',1);
    
    plot(REF(i,1:N)-X_K3(i,1:N),'-.','LineWidth',1);
    
    plot(REF(i,1:N)-X_K4(i,1:N),'-.','LineWidth',1);
    legend("AVPSMO-MPC","ATSMDO-MPC","NESO-MPC","DPSMO-MPC","SMDO-MPC");
%      xlim([0,N]);
%      ylim([-0.2,0.2]);
end

figure("name","Disturbance observation erorr")
for i = 1:3
    subplot (3,1,i);
    hold on;
    plot(f(i,1:N)-f_obs(i,1:N));
    plot(f(i,1:N)-f_obs1(i,1:N));
    plot(f(i,1:N)-f_obs2(i,1:N));
    plot(f(i,1:N)-f_obs3(i,1:N));
    plot(f(i,1:N)-f_obs4(i,1:N));
    legend("AVPSMO-MPC","ATSMDO-MPC","NESO-MPC","DPSMO-MPC","SMDO-MPC");
%      xlim([0,N]);
%      ylim([-0.05,0.05]);
end
...
subplot (3,1,1);
hold on;box on;
plot(0:0.1:0.1*(N-1),f(1,1:N),'-','LineWidth',2);
plot(0:0.1:0.1*(N-1),f_obs(1,1:N),'--','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs1(1,1:N),':','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs2(1,1:N),':','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs3(1,1:N),':','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs4(1,1:N),':','LineWidth',2)
ylabel('$f_{\rm x}$','FontSize',16,'Fontname','Times','Interpreter', 'latex');
legend("Disturbance","AVPSMO","ATSMDO","NESO","DPSMO","SMDO",'location','northwest','Interpreter', 'latex','NumColumns',6);
subplot (3,1,2);
hold on;box on;
plot(0:0.1:0.1*(N-1),f(2,1:N),'-','LineWidth',2);
plot(0:0.1:0.1*(N-1),f_obs(2,1:N),'--','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs1(2,1:N),':','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs2(2,1:N),':','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs3(2,1:N),':','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs4(2,1:N),':','LineWidth',2)
ylabel('$f_{\rm y}$','FontSize',16,'Fontname','Times','Interpreter', 'latex');
subplot (3,1,3);
hold on;box on;
plot(0:0.1:0.1*(N-1),f(3,1:N),'-','LineWidth',2);
plot(0:0.1:0.1*(N-1),f_obs(3,1:N),'--','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs1(3,1:N),':','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs2(3,1:N),':','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs3(3,1:N),':','LineWidth',2)
plot(0:0.1:0.1*(N-1),f_obs4(3,1:N),':','LineWidth',2)
ylabel('$f_{\rm \omega}$','FontSize',16,'Fontname','Times','Interpreter', 'latex');
xlabel('Time(s)','FontSize',16,'Fontname','Times');
hold off;

figure("name","Perturbation error integral");
c = categorical({'$f_{\rm x}$(m)','$f_{\rm y}$(m)','$f_\omega$(m)'});
c = reordercats(c,{'$f_{\rm x}$(m)','$f_{\rm y}$(m)','$f_\omega$(m)'});
y = [ferror1(1), ferror2(1), ferror3(1), ferror4(1), ferror5(1);
    ferror1(2), ferror2(2), ferror3(2), ferror4(2), ferror5(2);
    ferror1(3), ferror2(3), ferror3(3), ferror4(3), ferror5(3)];
bar(c,y);
legend("AVPSMO","ATSMDO","NESO","DPSMO","SMDO",'location','northwest','Interpreter', 'latex','NumColumns',6);

figure("name","Tracking error integral");
c = categorical({'$e_{\rm x}$(m)','$e_{\rm y}$(m)','$e_\omega$(m)'});
c = reordercats(c,{'$e_{\rm x}$(m)','$e_{\rm y}$(m)','$e_\omega$(m)'});
y = [perror1(1), perror2(1), perror3(1), perror4(1), perror5(1);
     perror1(2), perror2(2), perror3(2), perror4(2), perror5(2);
     perror1(3), perror2(3), perror3(3), perror4(3), perror5(3)];
bar(c,y);
%       ylim([0,2.1]);
legend("AVPSMO-MPC","ATSMDO-MPC","NESO-MPC","DPSMO-MPC","SMDO-MPC",'location','northwest','Interpreter', 'latex','NumColumns',3);